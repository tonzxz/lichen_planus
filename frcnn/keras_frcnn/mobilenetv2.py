...
def correct_pad(backend, inputs, kernel_size):
    ...
    # Returns
        A tuple.
    """
    img_dim = 1 # if backend.image_data_format() == 'channels_first' else 1
...
def nn_base(input_tensor=None, trainable=False,shape=320):
    ...
    channel_axis = axis #if K.image_data_format() == 'channels_first' else -1
...
def _conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1)):
    channel_axis = axis #if backend.image_data_format() == 'channels_first' else -1
