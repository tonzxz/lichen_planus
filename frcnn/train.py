    ...
    while True:
        ...
        for img_data in all_img_data:
            try:
                ...
                x_img = cv2.resize(x_img, (300, 400), interpolation=cv2.INTER_CUBIC)
                debug_img = x_img.copy()
                try:
                    y_rpn_cls, y_rpn_regr, num_pos = calc_rpn(C, img_data_aug, width, height, 300, 400, img_length_calc_function)
                except:
                    continue
...
if debug_num_pos==0:
    ...
    Y0_transpose = np.transpose(Y[0], (0, 2, 1, 3))
    Y1_transpose = np.transpose(Y[1], (0, 2, 1, 3))
    cls = Y0_transpose[0]
...
#                 print('Average number of overlapping bounding boxes from RPN = {} for {} previous iterations'.format(mean_overlapping_bboxes, epoch_length))
            ...
            # Convert rpn layer to roi bboxes
            R = rpn_to_roi(P_rpn[0], P_rpn[1], C, K.set_image_data_format('channels_last'), use_regr=True, overlap_thresh=0.7, max_boxes=300)
...
model_classifier.save('model/model_classifier.h5', include_optimizer=False)
model_rpn.save('model/model_rpn.h5', include_optimizer=False)

from tensorflow.keras.models import load_model

mdl = load_model('model/model_classifier.h5',custom_objects={"RoiPoolingConv" : RoiPoolingConv}) # Your model's name
converter = tf.lite.TFLiteConverter.from_keras_model( mdl) 
model = converter.convert()
file = open( 'model_classifier.tflite' , 'wb' ) 
file.write( model )
file.close()

mdl = load_model('model/model_rpn.h5') # Your model's name
converter = tf.lite.TFLiteConverter.from_keras_model( mdl ) 
model = converter.convert()
file = open( 'model_rpn.tflite' , 'wb' ) 
file.write( model )
file.close()
